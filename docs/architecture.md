...
- **portrait 顔枠（Issue #73 Phase 1）**: NPC に `portrait=path.png` が指定されているとダイアログ左側に 80×80 の顔枠を表示し、テキスト開始 x を顔枠右側（140px）にシフトする。VN 風の固定 1 枚。portrait 未指定時は従来どおり顔枠なし。画像ロードは `PIXI.Assets.load` + path 単位キャッシュ（`portraitCache`）。ロード失敗時は黒枠プレースホルダのみ表示。動的表情切替は Phase 2 (#101) スコープ
