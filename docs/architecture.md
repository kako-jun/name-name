...
**主人公の本文色（#305）**: `dialog_style: novel` で `protagonist` 指定時、主人公セリフの本文だけ暖アイボリー `#FFF6E6`、住人は純白。`NovelRenderer.resolveBodyTextColor(speaker)`（novel かつ話者 = protagonist のときだけ `protagonistTextColor`、それ以外・adv・protagonist 未指定・話者 null は `RESIDENT_TEXT_COLOR=0xffffff`）で**話者から決定論的に導出**し、`render()` の都度 `DialogBox.setBodyTextColor()` に渡す。DialogBox 側は `bodyTextColor`（既定白）を `dialogText.style.fill` と表示中ルビの fill に当てる。既定の主人公色は `#FFF6E6`、`NovelPlayer` → `NovelRenderer.setProtagonistTextColor()`（frontmatter `protagonist_text_color`、不正・空は既定にフォールバック）で per-game 上書き可。per-line の描画属性であり演出中間状態ではないため GameState には持たず、復元時も render で再導出される。adv / protagonist 未指定では全員白＝後方互換。
