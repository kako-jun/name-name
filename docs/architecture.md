...
- `Choice` — 選択肢（options: ChoiceOption[]。各 `ChoiceOption` は `text`/`jump` に加え任意の `condition?: string` を持つ。`- text → jump [条件: flag]` 構文で個別ロックを付けられる、#591。さらに任意の `cleared?: string` も持ち、`[完了: flag]` 構文でクリア済み視覚状態（選択可能なまま暗い配色）を付けられる、#594（#596でキーワードを`完了`に改名）。両構文は同じ行に併記可能。詳細は `docs/spec/markdown-v0.1.md` の「選択肢の条件付きロック」「選択肢の完了（クリア済み）視覚状態」参照）。GUI版・TUI版とも、ロック中選択肢はダイム配色のみで表し記号は付けない（旧🔒絵文字連結はGUI版が#598、TUI版が#609でそれぞれ撤去）。GUI版はさらに「ロック」と「既読/完了」を独立した2軸として扱いアイコンを出し分ける（`resolveChoiceIconKind`、`novelLayout.ts`、#598追記3/#604訂正。配色の優先順位 locked > cleared > alreadyRead > 通常 とは別軸で、`alreadyRead` は無関係。`locked` はアイコン種別の判定に一切関与しない——ロックは「読むことすらできない」状態を表すだけで「未読でない」わけではないため、#604 で `locked` 引数自体を関数シグネチャから削除した）: `cleared`（既読/完了、ロック中かどうかは無関係）はプロジェクト任意アセット `assets/images/read-icon.webp`、`!cleared`（未読、`alreadyRead` の真偽やロック中かどうかに関わらず）はプロジェクト任意アセット `assets/images/unread-icon.webp` を、それぞれ `ChoiceOverlay.setAssetBaseUrl()`（`NovelRenderer.setAssetBaseUrl` から他レイヤと同じタイミングで伝播）で `Assets.load()` 先読みし（favicon.png と同じ `assets/raw/{path}` 取得規約）、成功していればテキストの上にアイコンを表示する（未配置/取得失敗時は配色のみのフォールバック、read/unread それぞれ独立に判定）。アイコン表示時は `ChoiceOverlay.layoutButtonHeight` がアイコン+テキストの2段組みが収まる高さへ切り替わる（アイコンが一切描画されない回は通常の `BUTTON_HEIGHT` のまま非破壊）。アイコン/テキストの縦位置は純粋関数 `computeChoiceIconLayout`（`novelLayout.ts`）が算出する。GUI版はさらに #633 で `ChoiceOverlay.handleKeyDown()` によるキーボード操作（Tab/Shift+Tab・矢印キーでのフォーカス移動、Enter/Space での確定、visible focus ring）を持つ——`TitleScreenOverlay`（#633 同時対応）と同じパターンをグリッド配置（#508、同じ列/行内移動）・ロック（#591）・#339 スクロール追従に合わせて拡張したもので、`NovelRenderer.handleKeyDown` は `waitingForChoice` の間 window keydown をこのメソッドへ委譲する。**focus-visible化（#639）**: #633 実装当初は `show()` 時に無条件で先頭選択肢へフォーカスリングを描画しており、マウス/タップのみで進める大多数のプレイヤーにも黄色いリングが常時表示されてしまっていた。`ChoiceOverlay` に `keyboardNavActive` フラグを追加し、`moveFocus`/`moveFocusInColumn`/`moveFocusInRow`（Tab/矢印キー駆動）が呼ばれて初めてリングを可視化するよう修正（マウスクリックはこのフラグに触れない、ブラウザネイティブの `:focus-visible` と同じ考え方）。一度キーボード操作をしたセッションでは、以後の `show()` でも直ちにリングが出る（キーボードユーザーと判定したら以後も維持する設計）。同じパターンで A/S/D/設定等の素の HTML `<button>`（`NovelPlayer.tsx` 等）にも Tailwind の `focus-visible:` バリアントを適用し、ブラウザ標準の `:focus` アウトライン（マウスクリックでも出て残り続ける）を置き換えた。**`TitleScreenOverlay` にも同型バグが残っていたが、#640 で同じ `keyboardNavActive` パターンを移植して修正した**（`moveFocus()` が呼ばれて初めてリングを可視化する、マウスクリックはフラグに触れない、という設計をそのまま踏襲。`TitleScreenOverlay` は「はじめから」「設定」「終了」が無効化されない仕様のため、`ChoiceOverlay` にあった「選択肢1件のみでの早期returnエッジケース」は理論上到達不能だが、将来の変更に備え同じ防御ロジックは実装してある）。
