...
### NPC スプライトとアニメーション（TopDown）

`frontend/src/game/npcSpriteSheet.ts` にロード・切り出し・手続き生成の責務を集約。`TopDownRenderer` はシートを NPC 単位で非同期ロードし、完了したら placeholder の色付き四角を隠して `Sprite` に差し替える。
...
- **外部スプライト**: `PIXI.Assets.load(path)` で PNG 等をロード。失敗時は `console.warn` して placeholder（色付き四角）のまま維持
- **`__demo` センチネル**: サンプルデータが `sprite: '__demo'` を指定した場合、`buildDemoSheet` で `RenderTexture` に頭・胴体・足を描いた簡易スプライトを生成する。リポに画像アセットを持たずに「NPC が動く」デモを成立させるための仕組み
- **フォールバック**: `sprite` 未指定・ロード失敗・未対応形式はすべて従来どおり `色=` で指定した単色矩形で描画（赤枠付き）
