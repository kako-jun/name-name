...
- **対象は読ませる表示テキスト全部**: `Dialog.text` / `Narration.text` / `Choice.options[].text` / `TitleShow.text` / `Label.text`。**対象外**: RPG マスタ名（Monster / Item / Spell / PartyMember の name・Npc の name/message）・frontmatter・`##` 見出し（scene.title）・ディレクティブ引数・アセットパス・ID・話者名（`canonicalize_events` の match が分岐を持たず素通しする＝マスタ / ドメイン分離）。
