...
- **既知の制限**: ジャンプ中も `isPassable` 判定は変えない（現状壁は通行不可のまま、壁の上に乗る挙動は未実装）。床段差（floorHeights）・天井段差・Markdown 構文での jump 演出指定は別 PR で扱う（Issue #80 の他項目）。NPC 遮蔽連動・fogMaxDist 連動は Phase 2 で対応済み（本 PR、上記「既知の制限（Phase 1 → Phase 2 引き継ぎ）」参照）
