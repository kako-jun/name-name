...
- **レイヤ順**: `worldLayer` 直下に `worldGraphics`（空・床のベタ塗り、奥）、`wallSpritesContainer`（壁 Sprite プール、その手前）、`stepWallSpritesContainer`（段差壁面 Sprite プール、さらに手前）を配置。NPC は stage 直下の `npcLayer` にあるため、空 → 床 → 壁 → 段差壁 → NPC の順で重なる（段差壁は常に壁より手前なので、段差壁ピクセルは壁の上に描かれる）
...
- **段差の zBuffer 扱い**: 列ごとに段差のうち最も手前の depth が壁 depth より小さければ `zBuffer[i]` を段差 depth で上書きする。これにより NPC が段差と壁の間にいるとき、段差で遮蔽されて正しく前後関係が保たれる。段差は常に壁より手前（DDA は壁ヒットで止まる前の境界）なので、この上書きは「より厳しい遮蔽」として安全に働く
...
- **既知の制限**: 段差の u 座標は「ray と段差面の交差点のタイル内位相」を近似的に `computeWallU`（通常壁向けの側面 u 計算）で算出している。真に正しい u は段差境界の「長辺方向の位置」だが、MVP では視覚的違和感が小さいので採用。真の斜面（連続高低差を ray 射影する）や CornerHeights、Camera.pitch の本物化（ray に Z 成分）は Issue #99 で別途扱う。Markdown 構文 / エディタ UI からの `floorHeights` 指定も別 Issue
