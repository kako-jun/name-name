...
から読む（未指定は `INTERMISSION_FADE_MS_DEFAULT`=1500ms、通常既定 700ms より遅めの「幕が降りる」
値）。消去フェード完了後、intermission.md 自身のイベント列を `renderIntermissionTableau()` が
静止画タブローとして1回だけ描画して凍結する（`storyEnded=true` で以後 `advance()` は no-op の
ため、複数拍の演出は不要）。**intermission.md 未配置のプロジェクト**は、DOM 側（`NovelPlayer`）が
...
**登場フェードインも texture-ready ゲートに揃える（#17）**: 新規立ち絵の入場フェード（alpha 0→1）は、テクスチャ load 完了後に開始する。退場衝突がある経路（`evictCollidersAt` のコールバック）は元から texture-ready を待っていたが、**退場対象がいない（`colliderCount===0`）新規立ち絵は texture を待たず即フェードを開始していた**ため、初回コールドキャッシュで texture load が fade（立ち絵の登場フェードは `character_fade_ms`・エンジン既定 `DEFAULT_FADE_MS`=700ms〔#407 で 300→700〕）より遅いと、alpha が 1 に達し切ってから絵が現れ「フェードが見えず突然出る」不具合になっていた（例: ハブに居らず本編入口で初出の司会ヴィンチア＝衝突が無いためこの経路を踏む）。`colliderCount===0` でも `startEntrance`（texture-ready ゲート）を通すよう揃えて解消する。`instant`（skip / `applyState` 復元）は従来どおり即時表示（フェード無し）。
