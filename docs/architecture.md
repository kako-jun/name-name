...
`aspect_ratio`/`choice_style`/`font_family`/`font_size`/`dialog_style` の供給元（サブ MD の RPG シーンは未対応）。
...
フロントマターは Event ではなく、`Chapter` 構造体のフィールド（number, title, hidden, default_bgm）と `Document` ルートフィールド（engine, aspect_ratio, choice_style, font_family, font_size, dialog_style）としてパースされる。`Scene` も Event ではなくドキュメント構造の一部。
