...
**立ち絵のスケール（原寸 / 目標高さ / フィット・#360 #294）**: 立ち絵（`CharacterLayer.show()`）の表示スケールは `loadTexture` で3段の優先順位で決まる。①話者行に `フィット`/`fit` を書いた立ち絵は `computeFitScale`（論理画面より大きいときだけ画面全体に収める・#294）②per-game frontmatter `character_height_ratio` があれば `computeTargetHeightScale(textureHeight, ratio, screenHeight) = (ratio×screenHeight)/textureHeight` で目標表示高さに合わせて uniform スケール（幅はアスペクト比で追従・#360）③どちらも無ければ原寸 `scale=1`（後方互換）。②は**元画像の解像度に依らず画面上の高さを一定**にするため、立ち絵を高解像度で描き直しても大きさは不変で鮮明さだけ上がる（2倍リサイズした立ち絵が原寸だと画面をはみ出す問題への対処）。数式は `computeFitScale` / `computeTargetHeightScale`（ともに `CharacterLayer.ts` の export 純関数・非有限/非正入力は 1 にフォールバック）へ切り出してテストする（doctrine 規律4）。`character_height_ratio` は演出中間状態でなく per-game 表示設定なので GameState には持たず、`NovelPlayer` → `NovelRenderer.setCharacterHeightRatio()` → `CharacterLayer` に流す（`character_y_ratio` #308 と対称）。render-only（Title/Label/Image #274）は `loadTexture` を通らず自前 sizing のため非対象。
