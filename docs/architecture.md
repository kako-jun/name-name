...
- parse済み `EventDocument` cache: key は `projectName + ref + path + sha + schemaVersion`。
  ...
  変えた場合は `PARSED_SCRIPT_DOCUMENT_SCHEMA_VERSION` を上げて既存 cache を無効化する。
...
- `PageBreak` — 手動改頁マーカー（本文中の単独行 `---`。`dialog_style: novel` の強制ページ境界。非テキストイベントとして読み飛ばす。#292 Phase 2）
...
**登場フェードインも texture-ready ゲートに揃える（#17）**: 新規立ち絵の入場フェード（alpha 0→1）は、テクスチャ load 完了後に開始する。退場衝突がある経路（`evictCollidersAt` のコールバック）は元から texture-ready を待っていたが、**退場対象がいない（`colliderCount===0`）新規立ち絵は texture を待たず即フェードを開始していた**ため、初回コールドキャッシュで texture load が fade（既定 700ms）より遅いと、alpha が 1 に達し切ってから絵が現れ「フェードが見えず突然出る」不具合になっていた（例: ハブに居らず本編入口で初出の司会ヴィンチア＝衝突が無いためこの経路を踏む）。`colliderCount===0` でも `startEntrance`（texture-ready ゲート）を通すよう揃えて解消する。`instant`（skip / `applyState` 復元）は従来どおり即時表示（フェード無し）。
