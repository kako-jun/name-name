...
**TUI版との関係（#547 should3・誤解注意）**: TUI版にも `fullscreen_image` という語が出てくる機構（`tui/src/ui.rs` の `draw_splash`/`draw_fullscreen_image`）があるが、これは GUI 側の本機構とは**疎結合の別物**であり「対になる実装」ではない。TUI側は `Config.splash.logo_image`（ゲーム設定ファイル由来）が設定されている場合にのみ発火するスプラッシュ画面専用の描画モードで、共有 parser クレートに本節で説明している `Document.fullscreen_image`（script.md frontmatter 由来）フィールドが追加された後も、TUI 側のコードはこのフィールドを一切参照していない（TUI にとっては死んだフィールド）。一方 GUI 側のこの実装は `Document.fullscreen_image` を document 単位のフラグとして読み、script.md 中のあらゆる `[イベント絵:]` に効く（起動時スプラッシュに限らない）。トリガー元（ゲーム設定ファイル vs script.md frontmatter）も適用範囲（起動時1回だけ vs 全イベント絵）も異なる独立した機構である点に注意。

**テクスチャ拡大縮小フィルタ（pixel_art, #466）**: PixiJS は既定でテクスチャ拡大時に linear（滑らか）フィルタを掛けるため、Gymnasia の 128x128 ドット絵イベント絵を cover-fit で拡大表示するとブロック状のドットがぼける。frontmatter `pixel_art: true` で per-game に `'nearest'`（ドット感を保つ）へ切り替えられる（未指定/`false` は従来どおり `'linear'`＝theo-hayami 等の塗り絵作品は非回帰）。`NovelPlayer` → `NovelRenderer.setPixelArt(enabled)` → `CharacterLayer.setPixelArt()` / `EventImageLayer.setPixelArt()` の対称配線（`character_scale` 等と同じ流儀）で、値の所有権は各レイヤー側に置き renderer はフィールドを持たず素通しする。適用箇所はテクスチャロード完了後（`Assets.load().then()` 内）の `texture.source.scaleMode = pixelArt ? 'nearest' : 'linear'`（PixiJS v8 API）で、`CharacterLayer` は立ち絵（`loadTexture`）と render-only 単独画像（`showImage`、`[画像:]`）の両方に効く。**対象外**: `NovelRenderer` が内蔵する背景画像（`setBackground`/`showLoadedBackground`）は本設定の対象に含めていない。Gymnasia は `[背景:]` タグ自体を使わず全てイベント絵で構成する設計（#76）のため現状不要と判断した。将来ドット絵背景を使う作品が出た場合は同じパターン（`texture.source.scaleMode` の分岐）を `showLoadedBackground` に追加する形で拡張できる。`pixel_art` を後から変えても、`character_scale`（#378）と同じく表示中の sprite に即再適用される: `CharacterLayer.setPixelArt()` は `reapplyPixelArt()` で `this.characters` の全 sprite（立ち絵・render-only・クロスフェード中の旧 sprite 問わず、テクスチャロード済みのもの）の `scaleMode` をその場で書き換え、`EventImageLayer.setPixelArt()` は show() が保持する直近の `currentTexture` を同様に書き換える（scale 系のライブ再適用と異なりジオメトリを変えないため、アニメ中・fit 等の除外は不要）。editor で `pixel_art: true` を追記した瞬間に表示中の絵の見た目も揃う。**タイトル画面・終劇オーバーレイの title.png（#553）**: `TitleOverlay`（起動時タイトル画像）と `NovelPlayer` の終劇オーバーレイ（confinement 外脱出時に出す埋め込み元ロゴ）は PixiJS の外、DOM の素の `<img>` で `${assetBaseUrl}/images/title.png` を表示するため、上記の `scaleMode` 配線とは別経路で、`pixelArt` prop が true のときだけ `style={{ imageRendering: 'pixelated' }}` を直接その `<img>` に適用する（PixiJS レイヤーを経由しない、CSS の `image-rendering` によるブラウザ側の補間抑制）。値の出どころは同じ `Document.pixel_art` で、PlayerScreen → `NovelPlayer`/`TitleOverlay` へ他の per-game 設定と同じ経路で流れる。
...
`TitleOverlay` の「つづきから」ボタン（`PlayerScreen.tsx` の `onContinue`）は、この自動クイックロードとの整合を取るため `renderer.hasQuickSave()` で分岐する:
