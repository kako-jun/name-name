...
**立ち絵のスケール（原寸 / 目標高さ / per-character override / フィット・#360 #364 #294）**: 立ち絵（`CharacterLayer.show()`）の表示スケールは `loadTexture` で4段の優先順位で決まる。①話者行に `フィット`/`fit` を書いた立ち絵は `computeFitScale`（論理画面より大きいときだけ画面全体に収める・#294）②per-character frontmatter `character_height_ratios`（キャラごとの目標高さ override マップ・#364）に該当キャラがあれば、その値で `computeTargetHeightScale(textureHeight, ratio, screenHeight) = (ratio×screenHeight)/textureHeight` を適用して uniform スケール（幅はアスペクト比で追従）③マップに該当キャラが無ければ per-game frontmatter `character_height_ratio`（スクリプト単位デフォルト・#360）があれば同じ式で適用④どちらも無ければ原寸 `scale=1`（後方互換）。②③は**元画像の解像度に依らず画面上の高さを一定**にするため、立ち絵を高解像度で描き直しても大きさは不変で鮮明さだけ上がる（2倍リサイズした立ち絵が原寸だと画面をはみ出す問題への対処）。②③の優先順位判定（マップ override があればそれ、無ければスクリプト単位デフォルト）は `resolveCharacterHeightRatio(characterName, ratios, defaultRatio)`（`CharacterLayer.ts` の export 純関数）に切り出している。`ratios[characterName]` を素朴なブラケットアクセスすると prototype chain 経由でキャラ名が `constructor` 等と一致したときに関数オブジェクトを返してしまうため、共通ヘルパー `hasOwn()`（`ownProperty.ts`・#368 で全箇所を統一）で own-property のみを見て判定する。数式は `computeFitScale` / `computeTargetHeightScale`（ともに `CharacterLayer.ts` の export 純関数・非有限/非正入力は 1 にフォールバック）へ切り出してテストする（doctrine 規律4）。`character_height_ratio` / `character_height_ratios` は演出中間状態でなく per-game 表示設定なので GameState には持たず、`character_height_ratios` は `NovelPlayer` → `NovelRenderer.setCharacterHeightRatios()` → `CharacterLayer.setCharacterHeightRatios()` に流す（既存の `setCharacterHeightRatio`（#360）・`character_y_ratio`（#308）と対称の配線）。render-only（Title/Label/Image #274）は `loadTexture` を通らず自前 sizing のため非対象。

**主人公の本文色（#305）**: `dialog_style: novel` かつ `protagonist` 指定の作品（実質 theo のみ）で、主人公セリフの本文だけ暖アイボリー `#FFF6E6`、住人は純白。`NovelRenderer.resolveBodyTextColor(speaker)`（novel かつ話者 = protagonist のときだけ `protagonistTextColor`、それ以外・adv・protagonist 未指定・話者 null は `RESIDENT_TEXT_COLOR=0xffffff`）で**話者から決定論的に導出**し、`render()` の都度 `DialogBox.setBodyTextColor()` に渡す。DialogBox 側は `bodyTextColor`（既定白）を `dialogText.style.fill` と表示中ルビの fill（`makeRubyTextStyle()` に集約・#307）に当てる。主人公色は**固定で `#FFF6E6`**: parser は色を解析せず `NovelPlayer` も渡さないため、**frontmatter での色上書きは未実装**（本番経路は常に既定 #FFF6E6）。`NovelRenderer.protagonistTextColor` フィールド（既定 #FFF6E6）と `setProtagonistTextColor()` はテスト・将来拡張用の内部フックとして残す（呼ばなければ既定が効く）。per-line の描画属性であり演出中間状態ではないため GameState には持たず、復元時も render で再導出される。adv / protagonist 未指定では全員白＝後方互換。
