...
- **フォールバック契約**: `NaN` / `Infinity` / `0` 以下は `1.0` 扱い。`resolveFloorHeight` が負値を沈み込みとして許容するのと対照的で、`resolveCeilingHeight` は退化ケース（天井が床より下）で頭ぶつけ判定が破綻しないよう `1.0` に倒す防御的設計（`getWallHeight` と同じ fallback=1）
...
- **既知の制限**: 視覚的な天井レンダリング（per-column ceiling texture）は未対応。現状は頭ぶつけ挙動のみで、天井そのものは見えない。跳ね返り演出（反発係数・下向き初速付与）もなく、天井到達時は即 `VZ=0` で落下開始。天井が床より低い退化ケース（`playerGroundZ > 1` の床段差上に低天井を置く等）は `resolveCeilingHeight` が `1.0` にフォールバックするものの `maxJumpZ = 1 - playerGroundZ` が負になりうる。その場合は頭ぶつけ判定が即発動してジャンプが成立しないが MVP では許容。Markdown 構文 / エディタ UI からの `ceilingHeights` 指定も別 Issue
