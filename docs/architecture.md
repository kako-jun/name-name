...
**主人公の本文色（#305）**: `dialog_style: novel` かつ `protagonist` 指定の作品（実質 theo のみ）で、主人公セリフの本文だけ黄みのある暖アイボリー `#FFEAC6`、住人は純白。`NovelRenderer.resolveBodyTextColor(speaker)`（novel かつ話者 = protagonist のときだけ `protagonistTextColor`、それ以外・adv・protagonist 未指定・話者 null は `RESIDENT_TEXT_COLOR=0xffffff`）で**話者から決定論的に導出**し、`render()` の都度 `DialogBox.setBodyTextColor()` に渡す。DialogBox 側は `bodyTextColor`（既定白）を `dialogText.style.fill` と表示中ルビの fill（`makeRubyTextStyle()` に集約・#307）に当てる。主人公色は**固定で `#FFEAC6`**: parser は色を解析せず `NovelPlayer` も渡さないため、**frontmatter での色上書きは未実装**（本番経路は常に既定 #FFEAC6）。`NovelRenderer.protagonistTextColor` フィールド（既定 #FFEAC6）と `setProtagonistTextColor()` はテスト・将来拡張用の内部フックとして残す（呼ばなければ既定が効く）。per-line の描画属性であり演出中間状態ではないため GameState には持たず、復元時も render で再導出される。adv / protagonist 未指定では全員白＝後方互換。
