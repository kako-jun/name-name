...
- **Phase 2 として残課題**: 床段差（踏むと z が上がる）、Markdown 構文 / エディタ UI からの wallHeights 指定は別 Issue（#80）で扱う。pitch（上下視線）は #80 Phase 2-1、ジャンプ（z 方向の動き）は #80 Phase 2-2 で対応済み（下記参照）
- **既知の制限（Phase 1 → Phase 2 引き継ぎ）**:
  - **NPC 遮蔽が壁高さ非対応**: NPC を覆う mask は `zBuffer[stripeIdx]` の depth 比較のみで、低い壁（wallHeight=0.5）の奥にいる NPC が「壁の上に頭が出る」演出にならず完全に隠れる。サンプルマップでは低い壁の奥に NPC を置いていないので顕在化しないが、ユーザー定義マップで起きうる
  - **遠方カリングが壁高さ非依存**: `fogMaxDist + 0.5` での描画スキップ判定は `wallHeight` を見ないため、高い塔も普通の壁と同じ距離で消える。Phase 2 で「遠くの塔がランドマークとして見える」演出を入れる場合は `fogMaxDist * Math.max(1, wallHeight)` 等の伸長を検討
...
- **データモデル**: `RaycastRenderer.playerZ: number`（タイル単位、0 = 地面）と `playerVZ: number`（タイル/秒）。`load()` ごとに `playerZ=0, playerVZ=0` にリセット。`MapData` への永続化は別 Issue
- **物理パラメータ**: `jumpInitialV = 3.0` タイル/秒、`gravity = 12.0` タイル/秒^2。最高到達高 = `v^2 / (2g) = 9 / 24 = 0.375` タイル、滞空時間 = `2v/g = 0.5` 秒。控えめだが視点が変わる感覚は十分
- **更新ロジック**: `updateMovement(dt)` 末尾で「`playerZ > 0` または `playerVZ ≠ 0` のときだけ」`playerVZ -= gravity*dt; playerZ += playerVZ*dt`、`playerZ <= 0` で `playerZ=0, playerVZ=0` に着地スナップ
- **px オフセット変換**: `cameraZOffsetPx = round(playerZ * h/2)`。pitch と同じ符号規約（正で baseY が下シフト＝視点が上＝壁・NPC が下方向に見える）。`pitchOffsetPx + cameraZOffsetPx = totalYOffsetPx` を空・床ベタ塗りの分割位置と純粋関数（`computeWallYRange` / `projectNpcToScreen`）に渡す
...
- **既知の制限**: ジャンプ中も `isPassable` 判定は変えない（現状壁は通行不可のまま、壁の上に乗る挙動は未実装）。床段差（floorHeights）・天井段差・Markdown 構文での jump 演出指定・NPC 遮蔽連動・fogMaxDist 連動はいずれも別 PR で扱う（Issue #80 の他項目）
