...
`fullscreen_image: true`（#530）はイベント絵側のもう一つのレイアウトモードで、`split_layout`（領域を分けてクロップして覆う）とは逆に「クロップせずキャンバス全幅で見せる」ことを優先する。`EventImageLayer.setFullscreenMode(true)` で有効化すると、以後の `show()` は `splitLayoutRegion` を無視し `computeFullscreenImageFit`（`novelLayout.ts`、常に横幅いっぱいで contain）で sprite の幅/高さ/x を決め、y=0 起点で表示する。画像の高さがキャンバス高さを超える場合は追加の縮小をせず、`handleWheel`（マウスホイール、`clampFullscreenImageScrollY` でクランプ）による縦スクロールで見せる。`NovelRenderer` はイベント絵表示処理（`processDirective` の `EventImage` 分岐）で `this.fullscreenImage` が true のときだけ `DialogBox.hide()`/`ChoiceOverlay.hide()` を呼びテキストウィンドウを隠す（次の Dialog/Narration/Choice 表示で通常どおり復帰する可逆トグル）。TUI版の同名モード（`tui/src/ui.rs` の `draw_splash`、#530 PR #531）と対になる GUI 側実装。
