...
name-name.llll-ll.com   ← CF Pages（静的フロント、本ドキュメント）
  ↓ fetch
name-name-api.kako-jun.workers.dev   ← CF Worker（octokit プロキシ）
  ↓ GitHub REST API
...
成功すると `name-name-api.kako-jun.workers.dev` に publish される。

### 動作確認

```bash
curl https://name-name-api.kako-jun.workers.dev/api/projects
...
| 変数 | 値 |
|---|---|
| `VITE_API_URL` | `https://name-name-api.kako-jun.workers.dev` |
...
LOCAL_REPOS_BASE=~/repos/2025:~/repos/private \
