...
/// [`Playback::cursor`] が前回チェック時から変化していたら（＝新しい item に到達した瞬間）、
/// その item に紐づく `current_se_cues()` を出現順にすべて一度だけ再生する（#502）。
...
/// `last_cursor` を `Option<usize>` にしているのは、起動直後の初回呼び出し（実カーソル値は
/// 常に有効な `usize`）を「前回と異なる」として確実に一致させ、原稿冒頭の `[SE:]` も
/// 取りこぼさないため。
///
