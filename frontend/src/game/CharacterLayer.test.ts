...
describe('CharacterLayer pixel_art スケールモード（立ち絵 show 経路, #466）', () => {
  ...
  function mockLoadResolves(): void {
    vi.spyOn(Assets, 'load').mockResolvedValue({
      width: 200,
      height: 400,
      source: { scaleMode: 'linear' },
  ...
  it('C1: setPixelArt(true) 後の show() は texture.source.scaleMode が nearest になる', async () => {
    mockLoadResolves()
    const layer = new CharacterLayer(800, 450)
  ...
  it('C2: setPixelArt(false)（明示）の show() は texture.source.scaleMode が linear のまま', async () => {
    mockLoadResolves()
    const layer = new CharacterLayer(800, 450)
  ...
  it('C3: setPixelArt を一度も呼ばない（既定）show() は texture.source.scaleMode が linear（後方互換の既定値固定）', async () => {
    mockLoadResolves()
    const layer = new CharacterLayer(800, 450)
    // setPixelArt を呼ばない = frontmatter 未指定相当。
    layer.show('hero', 'normal', '中央', '/assets')
    await new Promise((resolve) => setTimeout(resolve, 0))
    const state = asInternals(layer).characters.get('hero')
    expect(state!.sprite.texture!.source!.scaleMode).toBe('linear')
  ...
  it('C4: setPixelArt(true) を呼んだ後、別の expression への再 show でも新しいロードが nearest を維持する', async () => {
    mockLoadResolves()
    const layer = new CharacterLayer(800, 450)
    layer.setPixelArt(true)
    layer.show('hero', 'normal', '中央', '/assets')
    await new Promise((resolve) => setTimeout(resolve, 0))
    expect(asInternals(layer).characters.get('hero')!.sprite.texture!.source!.scaleMode).toBe(
      'nearest'
    )

    // 別 expression への再 show（クロスフェード経由で新規 loadTexture が走る）。
    layer.show('hero', 'angry', '中央', '/assets')
    await new Promise((resolve) => setTimeout(resolve, 0))
    const state = asInternals(layer).characters.get('hero')
    expect(state!.sprite.texture!.source!.scaleMode).toBe('nearest')
  })
...
describe('CharacterLayer pixel_art スケールモード（render-only showImage 経路, #466）', () => {
  ...
  it('C6: setPixelArt(true) 後の showImage() は texture.source.scaleMode が nearest になる', async () => {
    vi.spyOn(Assets, 'load').mockResolvedValue(fakeTexture(10, 10) as never)
    const layer = new CharacterLayer(800, 450)
    layer.setPixelArt(true)
    layer.showImage({ id: 'avatar', path: 'a.png', assetBaseUrl: '/assets' })
    await flushPromises()
    const st = imageChars(layer).characters.get('avatar')!
    expect((st.sprite.texture as { source: { scaleMode: string } }).source.scaleMode).toBe(
      'nearest'
    )
  })

  it('C7: setPixelArt(false)/未設定の showImage() は texture.source.scaleMode が linear のまま', async () => {
    vi.spyOn(Assets, 'load').mockResolvedValue(fakeTexture(10, 10) as never)
    ...
    await flushPromises()
    const st = imageChars(layer).characters.get('avatar')!
    expect((st.sprite.texture as { source: { scaleMode: string } }).source.scaleMode).toBe('linear')

    // 明示 false でも同じ結果になることを確認する。
    const layer2 = new CharacterLayer(800, 450)
    layer2.setPixelArt(false)
    layer2.showImage({ id: 'avatar2', path: 'a.png', assetBaseUrl: '/assets' })
    await flushPromises()
    const st2 = imageChars(layer2).characters.get('avatar2')!
    expect((st2.sprite.texture as { source: { scaleMode: string } }).source.scaleMode).toBe(
      'linear'
    )
  })

  it('C8: 別インスタンスの setPixelArt 設定は互いに影響しない（インスタンス独立）', async () => {
    // mockResolvedValue は全呼び出しに同一オブジェクト参照を返してしまい、2 インスタンスが
    // 同じ texture.source を共有・上書きし合う偽陽性を生む。呼び出しごとに新しい texture を
    // 返す mockImplementation で、実運用（毎回別 Assets.load 結果）に近い形にする。
    vi.spyOn(Assets, 'load').mockImplementation(async () => fakeTexture(10, 10) as never)
    const pixelLayer = new CharacterLayer(800, 450)
    pixelLayer.setPixelArt(true)
    const smoothLayer = new CharacterLayer(800, 450)
    smoothLayer.setPixelArt(false)

    pixelLayer.showImage({ id: 'avatar', path: 'a.png', assetBaseUrl: '/assets' })
    smoothLayer.showImage({ id: 'avatar', path: 'a.png', assetBaseUrl: '/assets' })
    await flushPromises()

    const pixelState = imageChars(pixelLayer).characters.get('avatar')!
    const smoothState = imageChars(smoothLayer).characters.get('avatar')!
    expect((pixelState.sprite.texture as { source: { scaleMode: string } }).source.scaleMode).toBe(
      'nearest'
    )
    expect((smoothState.sprite.texture as { source: { scaleMode: string } }).source.scaleMode).toBe(
      'linear'
    )
  })

  // X1: Assets.load が reject するケースで、scaleMode 代入行（.then() 内）に到達せず例外も投げない。
  //   pixel_art: true を設定していても異常系の分岐（catch）には影響しないことを縛る。
  it('X1: setPixelArt(true) を設定していても Assets.load 失敗時は例外を投げず、scaleMode 代入行に到達しない', async () => {
    vi.spyOn(Assets, 'load').mockRejectedValue(new Error('load failed'))
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {})
    const layer = new CharacterLayer(800, 450)
    layer.setPixelArt(true)
    ...
    await flushPromises()
    expect(warnSpy).toHaveBeenCalledTimes(1)
    // .then() 未到達のため texture は初期値のまま（scaleMode 代入は起きない）＝ fadeAnimation も張られない。
    const st = imageChars(layer).characters.get('x')!
    expect(st.fadeAnimation).toBeNull()
  })
})
...
describe('CharacterLayer showLabel/showImage 退場フェード予約中の再表示 (#429)', () => {
  ...
  // 14: レース再現（should-2, #429 re-review）。
  //   1. 初回・非instant showLabel → state 生成、ensureFontLoaded().then(startEntranceFade) が
  //      pending microtask のまま残る。
  //   2. その .then() が解決する前に remove()（非instant）→ destroyOnComplete:true の退場フェードを予約。
  //   3. さらに .then() が解決する前に、#429 の再表示分岐が発火して destroyOnComplete:false の
  //      復活フェード（fromAlpha=現在alpha, toAlpha=1）に切り替わる（正しい復活処理）。
  //   4. ticker が進み、復活フェードが中間値まで進行する。
  //   5. ようやく 1 の .then() が発火。旧ガード（destroyOnComplete のみ判定）は素通りし、
  //      fadeAnimation を {fromAlpha:0, toAlpha:1, startMs:今} で丸ごと上書きしてしまう。
  //   6. さらに ticker を進めると、旧ガードでは fadeAnimation が 5 で startMs をリセットされて
  //      いるため、中間値だった alpha が tf≈0 相当まで巻き戻る。
  //   startEntranceFade のガードを「fadeAnimation の有無」に広げた修正後は、復活フェード
  //   （3 で張ったオブジェクト、参照も同一）がそのまま継続し、alpha は巻き戻らない。
  //   Ticker は実時間の deltaMS も加算する（ensureTicker 参照）ため、elapsedMs の厳密な数値は
  //   アサーションに使わず、参照同一性 + 単調増加（巻き戻らない）で判定する。
  it('showLabel: 予約中再showで張った復活フェード進行中にensureFontLoadedが解決しても、入場フェードで上書きされずalphaが巻き戻らない（should-2 再現）', async () => {
    let resolveFont!: () => void
    const pending = new Promise<void>((resolve) => {
      resolveFont = resolve
    })
    ...
    // 1: 初回・非instant showLabel。font 未解決の間は入場フェードはまだ張られない。
    layer.showLabel({ id: 'name', text: 'hi', fontFamily: 'sans-serif' })
    const st = show429Chars(layer).characters.get('name')!
    expect(st.fadeAnimation).toBeNull()

    // 2: font 解決前に remove()（非instant）→ 退場フェード（destroyOnComplete:true）を予約。
    layer.remove('name')
    expect(st.fadeAnimation!.destroyOnComplete).toBe(true)

    // 3: さらに font 解決前に再 showLabel → #429 の再表示分岐が発火し、復活フェード
    //   （destroyOnComplete:false）に切り替わる。
    layer.showLabel({ id: 'name', text: 'hi', fontFamily: 'sans-serif' })
    expect(st.fadeAnimation!.destroyOnComplete).toBe(false)
    const reviveFadeRef = st.fadeAnimation

    // 4: ticker を進め、復活フェード（durationMs=characterFadeMs 既定700）を 40% まで進行させる。
    const internal = layer as unknown as {
      animTicker: { update: () => void } | null
      elapsedMs: number
    }
    internal.elapsedMs += 280
    internal.animTicker?.update()
    const midAlpha = st.sprite.alpha
    expect(midAlpha).toBeGreaterThan(0)
    expect(midAlpha).toBeLessThan(1) // 中間値であることの前提確認（フェードが実際に進行した）

    // 5: ようやく 1 の ensureFontLoaded が解決する。修正後はここでガードにより無視され、
    //   fadeAnimation は 3 で張った復活フェードのまま（オブジェクト参照も同一）で残る。
    resolveFont()
    await flushPromises()
    expect(st.fadeAnimation).toBe(reviveFadeRef)

    // 6: さらに ticker をわずかに進める。修正後は復活フェードがそのまま継続するので、
    //   alpha は midAlpha から単調増加するだけで巻き戻らない（旧ガードなら fadeAnimation が
    //   5 で startMs リセットされ、ここで tf≈0 相当まで巻き戻っていたはず）。
    internal.elapsedMs += 1
    internal.animTicker?.update()
    expect(st.fadeAnimation).toBe(reviveFadeRef)
    expect(st.sprite.alpha).toBeGreaterThanOrEqual(midAlpha)
  })

  // 15a: レース再現・instant 復活サブケース（must-1, #429 2巡目 re-review, showLabel）。
  //   14 は「非instant 復活の revive fade が進行中」に古い .then() が発火するケースだったが、
  //   これは pending 中の再showが instant:true のケース。
  //   1. 初回・非instant showLabel → state 生成、ensureFontLoaded().then(startEntranceFade) が
  //      pending microtask のまま残る（fadeAnimation はまだ null）。
  //   2. その .then() が解決する前に remove()（非instant）→ destroyOnComplete:true の退場フェードを予約。
  //   3. さらに .then() が解決する前に instant:true で再 showLabel → reviveFromExitFade の
  //      instant 分岐が発火し、sprite.alpha=1・fadeAnimation=null に即時リセットされる
  //      （正しい即時表示。ここが 14 と違い、fadeAnimation は「捕捉時点(2)」と「復活後(3)」の
  //      両方で null になるため、fadeAnimation の有無だけを見るガードは復活が起きたことを検知できない）。
  //   4. ようやく 1 の .then() が発火する。旧ガード（fadeAnimation の有無）はここで
  //      state.fadeAnimation===null を素通りし、{fromAlpha:0, toAlpha:1, startMs:今} の新規入場
  //      フェードを張ってしまう。ticker が進むと sprite.alpha が 1 から一瞬 tf≈0 相当まで巻き戻り、
  //      その後 700ms かけて再度 1 へ戻る（見た目のチラつき）。
  //   reviveGeneration による修正後は、3 の復活で世代が進んでいるため 4 で expectedGeneration と
  //   不一致になり、新規入場フェードは張られない。fadeAnimation は null のまま、alpha は 1 のまま。
  it('showLabel: 予約中再showがinstant復活した後にensureFontLoadedが解決しても、新規入場フェードで上書きされない（must-1 instant復活サブケース再現）', async () => {
    let resolveFont!: () => void
    const pending = new Promise<void>((resolve) => {
      resolveFont = resolve
    })
    ...
    // 3: さらに font 解決前に instant:true で再 showLabel → instant 復活分岐が発火し、
    //   sprite.alpha=1・fadeAnimation=null に即時リセットされる（正しい即時表示）。
    layer.showLabel({ id: 'name', text: 'hi', fontFamily: 'sans-serif', instant: true })
    expect(st.sprite.alpha).toBe(1)
    expect(st.label!.alpha).toBe(1)
    expect(st.fadeAnimation).toBeNull()

    // 4: ようやく 1 の ensureFontLoaded が解決する。修正前はここで新規入場フェードが張られていた。
    resolveFont()
    await flushPromises()
    ...
    // 5: ticker を進めても alpha は巻き戻らない（旧ガードならここで tf≈0 相当まで巻き戻っていた）。
    const internal = layer as unknown as {
      animTicker: { update: () => void } | null
      elapsedMs: number
    }
    internal.elapsedMs += 10
    internal.animTicker?.update()
    expect(st.fadeAnimation).toBeNull()
    expect(st.sprite.alpha).toBe(1)
  })

  // 15b: 15a の showImage 版（must-1, #429 2巡目 re-review）。Assets.load の pending promise で
  //   同じ手順を踏む。label が無い点以外は 15a と同型のレース。
  it('showImage: 予約中再showがinstant復活した後にAssets.loadが解決しても、新規入場フェードで上書きされない（must-1 instant復活サブケース再現）', async () => {
    let resolveLoad!: (texture: unknown) => void
    const pending = new Promise((resolve) => {
      resolveLoad = resolve
    })
    ...
    // 1: 初回・非instant showImage。load 未解決の間は入場フェードはまだ張られない。
    layer.showImage({ id: 'avatar', path: 'a.png', assetBaseUrl: '/assets' })
    const st = show429Chars(layer).characters.get('avatar')!
    expect(st.fadeAnimation).toBeNull()

    // 2: load 解決前に remove()（非instant）→ 退場フェード（destroyOnComplete:true）を予約。
    layer.remove('avatar')
    expect(st.fadeAnimation!.destroyOnComplete).toBe(true)

    // 3: さらに load 解決前に instant:true で再 showImage → instant 復活分岐が発火し、
    //   sprite.alpha=1・fadeAnimation=null に即時リセットされる（正しい即時表示）。
    layer.showImage({ id: 'avatar', path: 'a.png', assetBaseUrl: '/assets', instant: true })
    expect(st.sprite.alpha).toBe(1)
    expect(st.fadeAnimation).toBeNull()
