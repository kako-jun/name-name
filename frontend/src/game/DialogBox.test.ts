...
//
// NovelRenderer が話者から導出した本文色（主人公=黄みのある暖アイボリー #FFEAC6 / 住人=白）を DialogBox に
...
describe('DialogBox setBodyTextColor (#305)', () => {
  ...
  it('setBodyTextColor(#FFEAC6 相当の数値) で dialogText.style.fill が更新される', () => {
    const box = makeRpgBox()
    box.setBodyTextColor(0xffeac6)
    expect(box.getBodyTextColor()).toBe(0xffeac6)
    expect(asInternals(box).dialogText.style.fill).toBe(0xffeac6)
    box.dispose()
  })
  ...
  it('表示中ルビにも本文色が当たる（本文色変更後の rubyEntries.style.fill）', async () => {
    ...
    for (const e of ruby) {
      expect(e.text.style.fill).toBe(0xffeac6)
    }
    box.dispose()
  })
})
