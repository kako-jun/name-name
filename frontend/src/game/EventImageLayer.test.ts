...
describe('EventImageLayer shouldHideBackLayer（可視性判定専用 API・セルフレビュー指摘）', () => {
  ...
  it('back=Hide のロード完了前（pending 中）は true のまま（既存の直ちに隠すタイミングは変えない）', () => {
    // このフィックスは「恒久的にロード失敗した」場合だけを救う狭いスコープ。ロード中はまだ
    // 失敗していないので true のまま = directive 処理と同時に背面を隠す既存タイミングは不変。
    mockAssetsLoadResolved()
    const layer = makeLayer(virtualTime())
    layer.show('story/x.webp')
