...
export interface ChoiceOption {
  ...
  condition?: string
  /** 完了(クリア済み)視覚状態 (#594、#596でキーワードを`完了`に改名)。`condition`
   *  （ロック）とは独立したフィールドで、同じオプションに両方指定できる。未定義 =
   *  完了しない（既存動作）。指定時は、そのフラグ名が真になると選択は可能なまま見た目
   *  だけ暗くなる。真偽判定は checkFlag と同じ規則。ロック中はロックの見た目が優先
   *  される（両方真になる想定は通常運用では無いが、防御的にロックを優先する）。 */
  cleared?: string
}
...
export type Event =
  ...
  | {
       ...
       * PixiJS レイヤーとしてキャラより手前・イベント絵/UIより奥に固定される点が異なる
