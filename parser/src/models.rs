...
pub enum Event {
    ...
    /// （runtime 側のリスト状態管理。パーサーは単に複数の `BackgroundBoard` イベントを
    /// 順番どおり emit するだけで、集約自体には関与しない）。
    ///
    /// シアターモードでは `depth` に応じてカメラから遠いほど縮小表示され
    /// （`computeCameraProjection`、#681/#682）、舞台の書割のような多層の奥行きを表現する。
    /// ノベルモードでは常に scale=1 で正面に重なるだけになる（#681 の設計により自動的に
    /// 後方互換、追加の分岐は不要）。
    ///
    /// クリアは既存 `Event::SceneTransition`（`[場面転換]`）にまとめる。専用のクリア
    /// ディレクティブは持たない（ランタイム側の状態管理で背景板リストを一括クリアする）。
    ///
    /// Markdown 構文: `[背景板: 空.png]` / `[背景板: 空.png, depth: 10]`。`depth` キーは
    /// コロン区切り（`parse_camera_mode_directive` の `向き:`/`仰角:` と同じ記法。
    /// docs/architecture.md のシアターモード構想節の例に合わせる）。値は非負の任意単位
    /// （シアターモードの基準距離は runtime 側の `THEATER_CAMERA_REFERENCE_DEPTH`）。
    /// 省略/非数値/NaN は `0.0`（最前面）にフォールバックする。厳密なバリデーション
    /// （負値クランプ等）はここでは行わずフロント側の描画時クランプに委ねる（Issue #683 方針）。
    BackgroundBoard {
        path: String,
        #[serde(default)]
        depth: f32,
    ...
    /// PixiJS レイヤーとしてキャラより手前・イベント絵/UIより奥に固定される点が異なる（数値 depth
    /// を背景板と共有する空間ではなく、レイヤー自体の描画順でグループ分けする——
    /// `奥 ← 背景板（depth複数）← キャラ ← 大道具（depth複数）← 手前`、
    /// docs/architecture.md「シアターモード構想」→「レイヤーモデル」節参照）。
    /// 1シーン内に複数書けば、それぞれ別の大道具として蓄積される（加算的、`BackgroundBoard`
    /// と同じ意味論）。
    ///
    /// シアターモードでは `depth` に応じてカメラから遠いほど縮小表示され
    /// （`computeCameraProjection`、#681/#682）、大道具どうしの奥行きを表現する。
    /// ノベルモードでは常に scale=1 で正面に重なるだけになる（`BackgroundBoard` と同じ設計）。
    ///
    /// クリアは既存 `Event::SceneTransition`（`[場面転換]`）にまとめる。専用のクリア
    /// ディレクティブは持たない。
    ///
    /// Markdown 構文: `[大道具: 机.png]` / `[大道具: 机.png, depth: 3]`。`depth` キーは
    /// `BackgroundBoard` と同じくコロン区切り。値は非負の任意単位（シアターモードの基準距離は
    /// runtime 側の `THEATER_CAMERA_REFERENCE_DEPTH`）。省略/非数値/NaN は `0.0`（最前面）に
    /// フォールバックする。厳密なバリデーション（負値クランプ等）はここでは行わずフロント側の
    /// 描画時クランプに委ねる（`BackgroundBoard` と同じ方針、Issue #692）。
    Prop {
        path: String,
        #[serde(default)]
        depth: f32,
...
pub struct Document {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub seekbar_color: Option<String>,
