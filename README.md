...
| 操作 | 動作 |
|---|---|
| Space / Enter | 次のテキストへ（選択肢表示中は確定、オーバーレイ表示中は閉じる） |
| ↑ / ↓ | 選択肢のカーソル移動（バックログ表示中はスクロール、設定画面表示中はフォーカス行の値を調整） |
| ← / → | 設定画面表示中のみ: フォーカス行の切り替え（テキスト速度/BGM音量/SE音量/ボイス音量） |
| a / A | オートモード（自動ページ送り）切り替え |
| s / S | スキップモード（既読テキスト早送り）切り替え |
| b / B | バックログ表示/非表示（表示中は↑/↓でスクロール） |
| c / C | 設定画面の表示/非表示（表示中は←/→で項目切替、↑/↓でテキスト速度またはBGM/SE/ボイス音量を調整） |
| q / Escape | 終了（オーバーレイ表示中は閉じる） |

※ GUI版とTUI版で`S`キーの意味が異なる（GUI: セーブメニュー / TUI: スキップモード）。`B`（バックログ）はGUI/TUI共通。
